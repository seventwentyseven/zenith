...
async def get_priv_badges():
    ...
    if Privileges.SUPPORTER in uprv:
        ...
        badges.append(["❤❤", "text-pink-500"])
    if Privileges.UNRESTRICTED not in uprv:
...
async def userGraph():
    ...
    for d in range(120):
        ...
        users.append([await app.state.services.database.fetch_val(
            "SELECT COUNT(id) AS `total`"
            "FROM users WHERE creation_time>:t1 AND creation_time<:t2",
            {"t1": t, "t2": t+86399}), date.strftime("%Y-%m-%d")])
    users.reverse()
    # Return data :D
    return {'success': True, 'result': users}
...
@api.route('/is_discord_connected', methods=['GET'])
async def isDiscordConnected():
    if 'authenticated' not in session:
        return {"success": False, "msg": "You are not logged in."}
    else:
        await updateSession(session)

    discord = await app.state.services.database.fetch_val(
    ...
    if discord:
        return {'success': True, 'result': discord}
    else:
        return {'success': False, 'result': False}

@api.route('/get_customization_data', methods=['GET'])
async def getCustomizationData():
    if 'authenticated' not in session:
        return {"success": False, "msg": "You are not logged in."}
    else:
        await updateSession(session)
    ...
    res = await app.state.services.database.fetch_one("""\
        SELECT u.id, u.preferred_mode, c.discord_tag,
          c.location, c.interests, c.website
        FROM users u
        LEFT JOIN customs c on u.id=c.userid
        WHERE u.id = :uid
        """,
        {"uid": session['user_data']['id']}
    )
    res = dict(res)
    res['color'] = session.get('color', None)
